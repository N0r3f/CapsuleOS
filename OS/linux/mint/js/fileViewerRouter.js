const MINT_VIEWER_BY_EXTENSION = {
...
const mintViewerState = {
    visionneur_images: null,
    visionneur_pdf: null,
    lecteur_multimedia: null
};

const getMintViewerTargetByExtension = (extension) => {
    if (!extension) {
        return null;
    }

    return MINT_VIEWER_BY_EXTENSION[String(extension).toLowerCase()] || null;
};

const getMintViewerTitle = (appId) => {
    const titles = {
...
const setWindowTitle = (appId) => {
    ...
    if (titleElement) {
        titleElement.textContent = getMintViewerTitle(appId);
    }
};
...
const renderMintViewer = (appId) => {
    const payload = mintViewerState[appId];
...
const openMintFileInViewer = (href, extension, name) => {
    const appId = getMintViewerTargetByExtension(extension);
    if (!appId) {
        return false;
    }

    mintViewerState[appId] = {
        href,
        extension: String(extension).toLowerCase(),
        name
    };

    const windowOpened = openMintWindow(appId);
    if (!windowOpened) {
        return false;
    }

    window.requestAnimationFrame(() => {
        renderMintViewer(appId);
    });
...
window.getMintViewerTargetByExtension = getMintViewerTargetByExtension;
window.openMintFileInViewer = openMintFileInViewer;
window.renderMintViewer = renderMintViewer;
